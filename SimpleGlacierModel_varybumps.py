...
if False:
    ...
    elayear = np.array([1500., 500., 1000, 1000., 500., 500., 500.])   # years  
###############################################################################
def get_bedrock(xaxis, a, b=10000, c=1000):
...
def run_sim(a, showplot = False):
    ...
    return time, elalist, elayear, response_time, dl_final, mass

###############################################################################
#Running and exporting to csv
###############################################################################

#name of csv file
filename = 'glacier_bump.csv'
filename_mass = 'Mass_bump.csv'

n_bumps = 9
amps = np.linspace(0, 160, n_bumps)
print(amps)

for j in range(n_bumps):
    filename_full = filename[:-4]+str(j)+filename[-4:]
    filename_mass_full = filename_mass[:-4]+str(j)+filename_mass[-4:]
    a = amps[j]
    time, elalist, elayear, response_time, dl_final, mass = run_sim(a)

    f = open(filename_full, 'w+')
    headers = ['h.ela','yr.ela', 't.response', 'dl_f', 'bump', 'bump height']

    csvobject = csv.writer(f, delimiter = ',', lineterminator='\n')
    csvobject.writerow(headers)

    for i in range(len(response_time)):
        csvobject.writerow([str(elalist[i]), str(elayear[i]), str(response_time[i]), str(dl_final[i]), str(bump), str(a)])
        print(i)
        
    f.close()

    f = open(filename_mass_full, 'w+')
    headers = ['time', 'mass', 'bump height']


    csvobject = csv.writer(f, delimiter = ',', lineterminator='\n')
    csvobject.writerow(headers)
    for i in range(len(time)):
        csvobject.writerow([str(time[i]), str(mass[i]), str(a)])
    f.close()
